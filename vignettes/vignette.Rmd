...
Now that the information about the diseases of interest was download from *CTDbase* we can access to it using the method `extract`. Method extract allows to access to different tables according to the origin of the \texttt{CTDquery} object. For a \textt{CTDquery} created from diseases the accessible tables are:
