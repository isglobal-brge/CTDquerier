...
ctd_genes <- query_ctd_gene( 
    terms = c( "APOE", "APOEB", "APOE2", "APOE3" , "APOE4" ) )
ctd_genes
```
...
Now that the information about the genes of interest was download from *CTDbase* we can access to it using the method `extract`. Methd extract allows to access to diferent tables according to the origin of the \code{CTDquery} object. For a \code{CTDquery} created from genes the accesible tables are:
