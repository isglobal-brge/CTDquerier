...
Now that the information about the genes of interest was download from *CTDbase* we can access to it using the method `extract`, from `psygenet2r` R package. Method extract allows to access to different tables according to the origin of the \code{CTDquery} object. For a \code{CTDquery} created from genes the accessible tables are:
...
With idea to allow user to create clean networks, arguments `subset.x` and `subset.y` can be used to filter genes and KEGG pathways. For GO term we use the same structure:
...
The argument `ontology` can take values `"Biological Process"`, `"Cellular Component"` and `"Molecular Function"`. One of them or any combination. This helps to filter the relation that will be plotted in the network. With the same idea, arguments `subset.x` and `subset.y` can be used to filter genes and GO terms.
...
Now that the information about the chemicals of interest was download from *CTDbase* we can access to it using the method `extract`. Method extract allows to access to different tables according to the origin of the \code{CTDquery} object. For a \code{CTDquery} created from chemicals the accessible tables are:
...
### Plotting Chemical Created `CTDquery` Objects

The generic `plot` function seen in the previous sections for *gene queries* also works with the *chemical queries*. The arguments that can be used are the same when ploting both types of queries.

The following plot shows the inference score for each chemical-gene association according to *CTDbase*.

```{r ctd_chem_hm_gene}
plot( ctd_chem, index_name = "gene", filter.score = 5 )
```
...
```{r ctd_chem_ntw_gene}
plot( ctd_chem, index_name = "gene", representation = "network",
...
The association between KEGG pathways and chemicals can be seen as a heat-map. The heat-map shows the P-Value of the association between ecach pathway and each chemical.

```{r ctd_chem_htm_kegg}
plot( ctd_chem, index_name = "kegg pathways", filter.score = 1e-40 )
```

The argument `filter.score` can be used to filter arrocations by their P-Value. Only the associations with a -Value lower or equal to the value given to `filter.score` are keep. Then, the heat-map calculates the terciles of the distribution of P-Values to create the legend (that has alwas four categories).

The heatmap for GO terms follows the same mechanic:

```{r ctd_chem_htm_go, fig.height=8}
plot( ctd_chem, index_name = "go terms",
    representation = "network", filter.score = 1e-230 )
```
...
Now that the information about the diseases of interest was download from *CTDbase* we can access to it using the method `extract`. Method extract allows to access to different tables according to the origin of the \code{CTDquery} object. For a \code{CTDquery} created from diseases the accessible tables are:
...
### Plotting Disease Created `CTDquery` Objects

```{r ctd_dise_htm_gen, fig.height=8}
library( gridExtra )
library( ggplot2 )
grid.arrange(
    plot( ctd_diseases, index_name = "gene", filter.score = 60, 
        subset.y = "Alzheimer Disease" ) + 
        ggtitle( "Genes associated to Alzheimer" ) +
        theme( legend.position = "top" ),
    plot( ctd_diseases, index_name = "gene", filter.score = 35, 
        subset.y = "Dementia" ) + 
        ggtitle( "Genes associated to Dementia" ) +
        theme( legend.position = "top" ),
    ncol = 2, nrow = 1
)
```
