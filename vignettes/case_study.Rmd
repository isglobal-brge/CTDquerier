...
gala_asthma <- gala_all_diseases[ gala_all_diseases$Disease.Name == "Asthma" , ]
dim( gala_asthma )
```
...
So, GALA genes are *associated* with 1'829 unique chemicals according to *CTDabse*. The `Reference.Count` of these *associations* can be ploted. First, lets see its distribution:

```{r, gala_chemicals_table}
table( gala_chem$Reference.Count )
```
...
plot( gala, index_name = "chemical", filter.score = 10 )
```
...
The table `gala_chem` contains teh relation between GALA genes and the chemicals associated to these genes according to *CTDbase*. The table `ctd_asthma_chem` contains the chemicals associated to Asthma accordint to *CTDbase*.
...
```
In other words, the 8.94% of the total association from GALA genes to *CTDbase* chemicals are shared with the Asthma related chemicals. Also that the 15.77% og the chemicals related to Asthma according to *CTDbase* are shared with the chemicals asociated with GALA genes.

```{r intersect_gala_asthma_chem_plot}
...
g1 <- ggplot(data = dta[1:25, ], aes(x = Chemical.Name, y = Reference.Gala)) +
  geom_bar(stat = "identity") + ggtitle("GALA") +
...
g2 <- ggplot(data = dta[1:25, ], aes(x = Chemical.Name, y = Reference.Asthma)) +xlab(NULL)+
  geom_bar(stat = "identity") + ggtitle("Asthma") +
...
library(grid)
g.mid<-ggplot(data = dta[1:25, ], aes(x=1,y=Chemical.Name))+geom_text(aes(label=Chemical.Name))+
  geom_segment(aes(x=0.94,xend=0.96,yend=Chemical.Name))+
  geom_segment(aes(x=1.04,xend=1.065,yend=Chemical.Name))+
  ggtitle("")+
  ylab(NULL)+
