...
The follofing code has the aim to create a `data.frame` (called `dta`) with only three columns: the chemical's name and their reference count in both GALA and Asthma chemicals sets.
...
In this case, `enrich` is takeing the genes in the `gala` object, the genes in the `asthma` object and all the genes in *CTDbase* and performing a Fisher test with these numbers (see `fisher.test`).
...
air
```
Once we obtained all th information about air pollutants from *CTDbase* we can use the already seen method `enrich` to see if the GALA genes are anriched in air Pollutants' chemicals.
