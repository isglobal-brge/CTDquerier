int_plot_chem_go_heatmap <- function( x, subset.x, subset.y, max.length = 30 ) {
  ...
  tbl <- data.frame( tbl )[ , c( "GO.Term.Name", "ChemicalName" ) ]
  tbl$Score <- 0
  goterm <- unique( tbl$GO.Term.Name )
  for( gg in chemicals ) {
    for( pp in goterm ) {
      if( nrow( tbl[ tbl$ChemicalName == gg & tbl$GO.Term.Name == pp,  ] ) == 0 ) {
        tbl <- rbind(tbl, c( pp, gg, 0 ) )
      } else {
        tbl[ tbl$ChemicalName == gg & tbl$GO.Term.Name == pp, "Score" ] <- 1
      }
    }
  }
  rm( gg, pp )

  ggplot2::ggplot( data.frame( tbl ), ggplot2::aes_string( x = "ChemicalName", y = "GO.Term.Name" ) ) +
    ggplot2::theme_bw() +
