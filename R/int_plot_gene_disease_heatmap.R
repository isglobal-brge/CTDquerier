int_plot_gene_disease_heatmap <- function( x, subset.x, subset.y,
    ...
    tbl <- psygenet2r::extract( x, name = "diseases" )
    ...
    if( length( diseases ) > 1 ) {
        ggplot2::ggplot( data.frame( tbl ), ggplot2::aes_string( x = "GeneSymbol", y = "Disease.Name" ) ) +
            ggplot2::theme_bw() +
        ...
        tbl$GeneSymbol <- factor(tbl$GeneSymbol, levels = tbl$GeneSymbol[ order( tbl[ , field.score], decreasing = TRUE ), ])
        ggplot2::ggplot( data.frame( tbl ), ggplot2::aes_string( x = "GeneSymbol", y = field.score ) ) +
            ggplot2::theme_bw() +
